...
export default function SWNPCSheet() {
  ...
  useEffect(() => {
    if (character && !loaded.current) {
      loaded.current = true;
      const { sw_attributes, sw_skills, ...rest } = character;
      setForm(rest);
  ...
  }, [characterId, form, swAttrs, swSkills, abilitiesEditor, notesEditor]);
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6">
        <div className="max-w-3xl mx-auto space-y-6">
              ...
              <div className="flex-1 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <Input value={(form.role_occupation as string) ?? ''} onChange={(e) => set('role_occupation' as any, e.target.value)} placeholder="e.g. Bandit Leader, Merchant" className="bg-muted border-border" />
                  </div>
                  <div>
          ...
          <section className="bg-card border border-border rounded-lg p-4">
            ...
            <div className="grid grid-cols-5 gap-2">
              {SW_ATTRIBUTES.map((attr) => (
                <div key={attr} className="text-center bg-muted rounded-lg p-2">
                  ...
                  <Select value={swAttrs[attr] ?? 'd4'} onValueChange={(v) => { setSwAttrs((p) => ({ ...p, [attr]: v })); setSaveStatus('idle'); }}>
                    <SelectTrigger className="bg-background border-border h-8 text-sm font-heading justify-center"><SelectValue /></SelectTrigger>
                    <SelectContent>
          ...
          {/* Derived Stats */}
          <section className="bg-card border border-border rounded-lg p-4">
            <h3 className="font-heading text-sm text-gold mb-3">Derived Stats</h3>
          ...
          {/* Key Skills */}
          <section className="bg-card border border-border rounded-lg p-4">
            <h3 className="font-heading text-sm text-gold mb-3">Key Skills</h3>
            <div className="space-y-2">
              {swSkills.map((skill, i) => (
                <div key={i} className="flex items-center gap-2">
                  <Input value={skill.name} onChange={(e) => { const c = [...swSkills]; c[i] = { ...c[i], name: e.target.value }; setSwSkills(c); setSaveStatus('idle'); }} placeholder="Skill name" className="bg-muted border-border flex-1" />
          ...
          <section className="bg-card border border-border rounded-lg p-4" style={{ backgroundColor: 'hsl(225 20% 15%)' }}>
            <div className="flex items-center justify-between mb-3">
              ...
              {canEdit && (
                <div className="flex items-center gap-2">
                  <Checkbox id="sw-npc-dm-notes-visible" checked={!!form.dm_notes_visible} onCheckedChange={(checked) => set('dm_notes_visible' as any, !!checked)} />
                  <Label htmlFor="sw-npc-dm-notes-visible" className="text-xs text-muted-foreground cursor-pointer">Visible to Players</Label>
                </div>
              )}
            </div>
